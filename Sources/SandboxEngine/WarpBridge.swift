...
/// State of the WARP VPN connection inside the guest VM.
public enum WarpState: Equatable {
    /// Initial state — haven't heard from the agent yet.
    case unknown
    /// WARP is connected and routing traffic.
    case connected
    /// WARP is in the process of connecting (e.g. happy eyeballs).
    case connecting
    /// WARP is not connected (but could be started).
    case disconnected
    /// warp-cli / warp-svc binaries are not installed in the VM.
    case notInstalled
