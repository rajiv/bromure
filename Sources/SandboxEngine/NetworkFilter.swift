...
public final class NetworkFilter: @unchecked Sendable {
    ...
    public init?(networkInfo: HostNetworkInfo, dnsOverrideServers: [UInt32] = []) {
    ...
    private func rewriteDHCPDNS(_ buf: UnsafeMutablePointer<UInt8>, count: Int) -> Data? {
        ...
        if dnsDebug {
            ...
            print("[DNS] DHCP response from \(HostNetworkInfo.formatIPv4(srcIP)) for client \(HostNetworkInfo.formatIPv4(clientIP)): DNS \(oldDNS) → \(newDNS) (packet \(count)→\(data.count) bytes)")
        }
