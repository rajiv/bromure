...
public final class LinuxImageManager {
    ...
    private static func setupScript(
        ...
        return [
            ...
            "",
            "# If DNS fails, switch to Cloudflare DNS and retry",
            #"if ! wget -q -O /dev/null --spider http://dl-cdn.alpinelinux.org/alpine/ 2>/dev/null; then echo 'DNS resolution failed, switching to Cloudflare DNS (1.1.1.1 / 1.0.0.1)'; printf 'nameserver 1.1.1.1\nnameserver 1.0.0.1\n' > /etc/resolv.conf; for i in $(seq 1 10); do wget -q -O /dev/null --spider http://dl-cdn.alpinelinux.org/alpine/ 2>/dev/null && break; sleep 1; done; fi"#,
            ...
            "",
            "# DNS — use Cloudflare DNS to avoid inheriting broken host resolver",
